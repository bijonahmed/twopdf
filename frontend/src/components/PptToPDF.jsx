...
const PptxToHtmlPreview = ({ description }) => {
  ...
  const handleConvert = () => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        const slideXmlFiles = Object.keys(zip.files).filter(fileName => fileName.startsWith('ppt/slides/slide') && fileName.endsWith('.xml'));

        const slidesData = [];
        const promises = slideXmlFiles.map(async (slideXmlFile, index) => {
          const xmlContent = await zip.files[slideXmlFile].async('string');
          const slideContent = await extractTextAndImagesFromXml(xmlContent, zip);
          slidesData.push({
            slideNumber: index + 1,
  ...
  const extractTextAndImagesFromXml = async (xmlContent, zip) => {
    return new Promise((resolve, reject) => {
      ...
      for (let i = 0; i < slideElements.length; i++) {
        const pictureElement = slideElements[i].getElementsByTagName('p:pic')[0];
        if (pictureElement) {
          const imagePath = pictureElement.getElementsByTagName('p:blipFill')[0].getElementsByTagName('a:blip')[0].getAttribute('src');
          if (imagePath) {
            const imageFileName = imagePath.split('/').pop();
            if (zip.files[`ppt/media/${imageFileName}`]) {
              zip.files[`ppt/media/${imageFileName}`].async('base64').then(function (imgData) {
    ...
    <div className="container">
      ...
      <div className="card mb-4 mt-5">
        <div className="card-body text-center">
          <h5 className="card-title">Upload PPTX File</h5>
          <input 
            type="file" 
      ...
      <div className="card mb-4">
        <div className="card-body text-center">
          <button className="btn btn-success w-100" onClick={handleExportPDF} disabled={slides.length === 0}>
            Export to PDF
          </button>
        </div>
      </div>
